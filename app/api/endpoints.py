...
while maintaining strict separation of concerns from business reasoning logic.
"""
...
from app.utils.constants import DEFAULT_STADIUM_ID, STADIUM_DISPLAY_NAME, STADIUM_ZONE_DATABASE
...
async def analyze_crowd_context(
    ...
    """
    try:
        action_plan: CrowdActionPlanResponse = await service.generate_crowd_action_plan(request)
        return action_plan
    except Exception as exc:
        logger.error(f"Error executing crowd intelligence analysis: {exc}", exc_info=True)
        raise HTTPException(
...
async def compute_spatial_enrichment(request: CrowdContextRequest) -> Dict[str, Any]:
    ...
    """
    try:
        enriched_data: Dict[str, Any] = SpatialContextEngine.enrich_context(request)
        return {
        ...
        logger.error(f"Error computing spatial enrichment calculation: {exc}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def evaluate_batch_csv(request: BatchCSVRequest) -> Dict[str, Any]:
    ...
    """
    try:
        results: Dict[str, Any] = SpatialContextEngine.evaluate_csv_batch(request.csv_payload)
        return {
            "status": "BATCH_PROCESSED_SUCCESSFULLY",
            "stadium_id": DEFAULT_STADIUM_ID,
            "batch_evaluation_results": results,
        }
    except ValueError as val_exc:
        logger.warning(f"Bad request in batch CSV upload: {val_exc}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(val_exc))
    except HTTPException as http_exc:
        raise http_exc
    except Exception as exc:
        logger.error(f"Unhandled failure processing batch CSV payload: {exc}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def health_check(
    ...
    """
    is_ai_live: bool = service.client is not None and bool(service.settings.gemini_api_key)
    return {
        "status": "ONLINE",
...
async def get_stadium_zones() -> Dict[str, Any]:
    ...
    for z_id, z_data in STADIUM_ZONE_DATABASE.items():
        zone_list.append(
            {
                "zone_id": z_id,
                "name": f"{z_data['architectural_profile']} ({z_id})",
                "type": z_data["architectural_profile"],
                "nominal_capacity": z_data["nominal_capacity"],
                "bottleneck_sensitivity": z_data["bottleneck_sensitivity"],
            }
        )

    return {
        "stadium_id": DEFAULT_STADIUM_ID,
        "stadium_name": STADIUM_DISPLAY_NAME,
        "zones": zone_list,
    }
